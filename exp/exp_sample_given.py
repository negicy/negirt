...
filename = 'results/result_20231214_175639.pickle'

with open(filename, 'rb') as p:
    results = pickle.load(p)

worker_list_alliter = results['worker_list_alliter']
task_list_alliter = results['task_list_alliter']
print(worker_list_alliter)
print(task_list_alliter)
...
for iteration in range(0, iteration_time):
    ...
    '''
    print(worker_list_alliter)
    test_worker = worker_list_alliter[iteration]
    test_task = task_list_alliter[iteration]
    qualify_task = list(set(task_list) - set(test_task))
    ...
    DI_onepl_output = DI_make_candidate_onepl_margin(
        threshold, input_df, label_df, worker_list, test_worker, qualify_task, test_task
    )
    ...
    for candidate_dic in DI_onepl_margin_candidate.values():
        ...
        for task in test_task:
            if task not in PI_onepl_assign_dic_opt.keys():
                ...
                if len(PI_onepl_sub_workers[task]) > 0:
                    PI_onepl_assign_dic_opt[task] = random.choice(PI_onepl_top_workers[:NA_num])
                else:
...
result_onepl_margin_acc_dic = {
  'DI': DI_onepl_margin_acc, 'top': top_acc, 'AA': AA_acc, 'random': random_acc, 'PI': DI_onepl_margin_acc,
  'DI_std': DI_onepl_margin_acc_std, 'top_std': top_acc_std, 'AA_std': AA_acc_std, 'random_std': random_acc_std, 'PI_std': DI_onepl_margin_acc_std
  }

result_onepl_margin_var_dic = {
  'DI': DI_onepl_margin_var, 'top': top_var, 'AA': AA_var, 'random': random_var, 'PI': DI_onepl_margin_var,
  'DI_std': DI_onepl_margin_var_std, 'top_std': top_var_std, 'AA_std': AA_var_std, 'random_std': random_var_std, 'PI_std': DI_onepl_margin_var_std
}

result_onepl_margin_tp_dic = {
  'DI': DI_onepl_margin_tp, 'top': top_tp, 'AA': AA_tp, 'random': random_tp, 'PI': DI_onepl_margin_tp
}
