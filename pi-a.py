...
for iteration in range(0, iteration_time):
  ...
  for th in PI_candidate:
    ...
    for task in test_task:
      if task not in PI_assign_dic_opt.keys():
        if len(ours_candidate[th][task]) == 0:
          assigned_worker = random.choice(top_workers)
        else:
          assigned_worker = random.choice(ours_candidate[th][task])
    ...
    # 割当て結果の精度を求める
    acc_PI = accuracy(PI_assign_dic_opt_A, input_df)
    # acc = accuracy(PI_assign_dic_opt_A, input_df)
   
    PI_margin_sum = 0
    for task in PI_assign_dic_opt_A:
      worker = PI_assign_dic_opt_A[task]
      PI_margin = full_user_param[worker] - full_item_param[task]
      PI_margin_sum += PI_margin

    PI_margin_mean = PI_margin_sum / len(PI_assign_dic_opt_NA)
 
    var_PI = task_variance(PI_assign_dic_opt_A, test_worker)
    tp_PI = calc_tp(PI_assign_dic_opt_A, test_worker)
    
    PI_acc_perth.append(acc_PI)
    PI_var_perth.append(var_PI)
    PI_tp_perth.append(tp_PI)
  ...
  for th in ours_candidate:
    ...
    for task in test_task:
      ...
      if task in PI_assign_dic_opt_NA.keys():
        DI_assign_dic_PI_NA[task] = DI_assign_dic_opt[task]
        PI_assign_dic_opt_NA[task] = DI_assign_dic_opt[task]
      if task in PI_assign_dic_opt_A.keys():
    ...
    acc_DI = accuracy(DI_assign_dic_PI_NA, input_df)
    # print("DI assignment", th, acc, len(assign_dic_opt))
    DI_margin_sum = 0
    
    for task in DI_assign_dic_PI_A:
      worker = DI_assign_dic_PI_A[task]
    ...
    DI_mean = DI_margin_sum / len(DI_assign_dic_PI_NA)
    # print(f'DI_margin_sum: {DI_margin_sum}')
    # print(DI_margin_mean)
    # 割当て結果分散を求める
    var_DI = task_variance(DI_assign_dic_PI_A, test_worker)
    # 割当て結果のTPを求める
    tp_DI = calc_tp(DI_assign_dic_PI_A, test_worker)

    ours_acc_perth.append(acc_DI)
    ours_var_perth.append(var_DI)
    ours_tp_perth.append(tp_DI)
...
result_plot_PIDI(threshold, result_acc_dic, ay='accuracy', bbox=(0.150, 0.400)).show()
result_plot_PIDI(threshold, result_var_dic, ay='variance', bbox=(0.150, 0.800)).show()
