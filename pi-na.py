...
for iteration in range(0, iteration_time):
  ...
  for th in PI_candidate:
    ...
    top_workers = sort_test_worker(test_worker, full_user_param, N=0)
    count = 0
    ...
    # 割当て結果の精度を求める
    acc_PI = accuracy(PI_assign_dic_opt_NA, input_df)
    ...
    var_PI = task_variance(PI_assign_dic_opt_NA, test_worker)
    tp_PI = calc_tp(PI_assign_dic_opt_NA, test_worker)
  ...
  for th in ours_candidate:
    ...
    # 割当て結果分散を求める
    var_DI = task_variance(DI_assign_dic_PI_NA, test_worker)
    # 割当て結果のTPを求める
    tp_DI = calc_tp(DI_assign_dic_PI_NA, test_worker)
