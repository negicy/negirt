...
user_param_list = norm.rvs(loc=1.5, scale=0.75, size=worker_size)
item_param_list = norm.rvs(loc=0.8, scale=0.75, size=task_size)

bins=np.linspace(-3, 3, 20)
plt.hist([user_param_list, item_param_list], bins, label=['worker', 'task'])

a_list = np.random.rand(100) + 0.5
user_param = {}
item_param = {}
...
for iteration in range(0, iteration_time):
  ...
  '''
  # 20231102: シミュレーション用の調整が必要
  ours_output = make_candidate(
        threshold, input_df, worker_list, test_worker, qualify_task, test_task, full_item_param
    )
  ours_candidate = ours_output[0]
  DI_user_param = ours_output[1]
  DI_item_param = ours_output[2]
